...
export default function CloudinaryPicker({value,onChange,label="Upload media",folder="phenomenal-shoots"}:{value:string;onChange:(url:string)=>void;label?:string;folder?:string}){
 ...
 async function upload(file?:File){if(!file)return;setBusy(true);setError("");try{const result=await uploadPublicToCloudinary(file,folder);onChange(result.secure_url);}catch(e){setError(e instanceof Error?e.message:"Upload failed")}finally{setBusy(false)}}
